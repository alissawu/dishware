...
function ProductCardBase({ item, qty, unit, discounted, compact, onQty, onOpen }: Props) {
  const active = qty > 0
  ...
  if (compact) {
    return (
      ...
      >
        <button onClick={onOpen} className="relative aspect-square overflow-hidden bg-[#efe8db]" aria-label={`View ${item.name}`}>
