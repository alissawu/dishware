...
function ListRowBase({ item, qty, unit, discounted, href, onQty, onOpen }: Props) {
  ...
  return (
    <div
      ...
      className={`group flex cursor-pointer items-center gap-2.5 rounded-xl border px-2.5 py-2 transition-colors sm:gap-3 ${
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <a
            ...
            className="flex-none text-muted/60 transition-opacity hover:text-clayDark sm:opacity-0 sm:group-hover:opacity-100"
          >
        ...
        <div className="truncate text-xs text-muted">
          {item.color}{item.dims ? ` · ${item.dims}` : ''}
        </div>
      </div>
      ...
      <div className="hidden w-16 flex-none text-right font-display text-sm tabular-nums text-ink sm:block">
